...
def find_sum(filename: str, target_sum: int, num_to_total: int) -> None:
  ...
  Description:  This function will open a file and scan it, looking for a pair
                of numbers whose sum is equal to target_sum.  It will then
                return the product of those two numbers.

  Input:        filename: str - Input filename

                target_sum: int - Sum total that we are searching for in the
                                  report.

                num_to_total: int - The number of values to use in calculating
                                    the sum.
...
def validate_num(value: int) -> int:
  ...
  if 2 > ivalue or 10 < ivalue:
    raise argparse.ArgumentTypeError(value, 'is an invalid option.  Must be an \
...
if __name__ == '__main__':
  ...
  parser.add_argument('--sum', '-s', type=int, metavar='sum', default='2020', \
                      help='Alternate sum value (default is 2020).')
  parser.add_argument('--num', '-n', type=validate_num, metavar='number', \
                      default='2', help='Number of values to sum`')
